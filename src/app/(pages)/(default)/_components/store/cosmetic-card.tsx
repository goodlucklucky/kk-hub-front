...
const CosmeticCard: React.FC<CosmeticCardProps> = ({
  ...
  return (
    <div className="flex flex-col gap-1 rounded-[10px] border border-[rgba(145,115,119,0.20)] bg-gradient-to-b from-[#FDE9C7] to-[#F5D6B1] shadow-[0px_2px_0px_0px_rgba(0,0,0,0.20)] p-1 relative" onClick={onClick}>
      <Image src={image} alt={title} className='aspect-[2/1.5]' />
      {isHot && (
      ...
      <div className="flex flex-col justify-center items-center py-0.5 gap-0 bg-[#E3BEAA] rounded-[5px] relative">
        <div className="text-[#5F3F57] text-[12px]/[14px] font-bumper-sticker font-normal px-2 text-center pt-0.5">{title}</div>
        <div className="text-[#5F3F57] text-[12px]/[14px] font-bumper-sticker font-normal px-2 text-center">{subtitle}</div>
        {showInfo && (
      ...
      <div className="bg-[#917377] rounded-[5px] flex items-center justify-center gap-x-1 py-1">
        <DollarIcon />
        <span className="text-[#F7DAB5] font-made-tommy font-medium text-[15px]/[15px]">{price.toFixed(2)}</span>
      </div>
    </div>
  );
};
