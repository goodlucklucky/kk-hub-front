...
type NoWinMessageProps = {
  onClose: () => void;
};
export const NoWinMessage = ({ onClose }: NoWinMessageProps) => {
  return (
    <div className="w-full flex flex-col justify-center items-center relative gap-y-2 py-1 px-3">
      <span className="text-[#8F6E75] text-center font-made-tommy text-[14px] font-bold leading-normal">
        Perhaps luck is waiting for you in the next draw... Go claim your next
        free ticket!
      </span>
      <Button
        onClick={onClose}
        className="rounded-[8px] border border-[#9C7B8F] bg-[#653F56] shadow-[0px_2px_0px_0px_rgba(0,0,0,0.20)] w-full flex gap-x-1 items-center justify-center py-[3px]"
      >
